...
const PageButton = ({ id, page, label, closeOverlay }) => {
  if (id === 6) {
    return (
      <button className={styles.selectedButton} type='button'>
...
const PageNav = ({ stage, currentPage, changePage, submitted }) => {
  ...
  const menu = (
    <div className={styles.container}>
      <div className={styles.logo}>{/* <Logo className={styles.logo}/> */}</div>
      <div className={styles.pageButtons}>{buttons}</div>
  ...
  if (isMobile) {
    return (
      <>
        <FontAwesomeIcon className={styles.menuButton} icon={faBars} size='3x' onClick={() => toggleOverlay(true)} />
        <div style={{ display: showOverlay ? 'flex' : 'none' }} className={styles.overlay}>
          {menu}
          <div style={{ width: '100%', height: '100%' }} onClick={() => toggleOverlay(false)} />
        </div>
      </>
    )
  } else {
